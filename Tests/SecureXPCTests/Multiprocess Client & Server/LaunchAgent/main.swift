...
func main() throws -> Int {
    ...
    server.registerRoute(SharedRoutes.echoRoute) { _, string in string }
    ...
    server.registerRoute(SharedRoutes.latestRoute) { _ in latestAndGreatest }
    server.registerRoute(SharedRoutes.mutateLatestRoute) { _ in
    ...
    server.registerRoute(SharedRoutes.terminate) { _ in
        exit(0)
    }
    server.registerRoute(SharedRoutes.fibonacciRoute) { _, n, provider in
        fibonacciSequence(n: n, provider: provider)
        provider.finished()
    }

    server.registerRoute(SharedRoutes.selfTerminatingFibonacciRoute) { _, n, provider in
        fibonacciSequence(n: n, provider: provider)
